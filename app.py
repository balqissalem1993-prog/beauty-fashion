"""
Beauty Fashion Store - Flask App
واجهة الزبون + الإدارة + نظام الأقسام
"""
...
app = Flask(__name__)
app.secret_key = "beautyfashion-secret-2026"

# ⚠️ غيّر هذين القيمتين بعد إنشاء مشروع Supabase الجديد (راجع database_setup.sql)
SUPABASE_URL = "https://iwdklcixngqafytpypyz.supabase.co"
SUPABASE_KEY = "sb_publishable_wVRvRIIGzy5ZXj7VGauUOA_ZZgtXoyB"
supabase = create_client(SUPABASE_URL, SUPABASE_KEY)

def safe_select(table_name, retries=3, delay=1):
    """يعيد محاولة الاتصال بـ Supabase عند فشل الشبكة المؤقت"""
    last_err = None
    for attempt in range(retries):
        try:
            ...
            last_err = e
            time.sleep(delay)
    print(f"⚠️ فشل الاتصال بجدول {table_name}: {last_err}")
    return []
...
def get_settings():
    ...
    return {"store_name":"Beauty Fashion","welcome_message":"أناقتك تبدأ من هنا",
            "whatsapp_number":"218910000000","logo_url":"","btn_text":"أضف للسلة",
            "color_primary":"#d4af37","color_accent":"#1a1a1a",
            "color_bg":"#faf9f7","color_text":"#1a1a1a",
...
def load_users():
    ...
    return {"Admin": {"password": "Beauty2026", "role": "admin"}}
...
def get_categories():
    cats = safe_select("categories")
    return sorted(cats, key=lambda c: c.get("display_order", 0))
...
def store():
    ...
    if not s.get("maintenance_mode"):
        all_products = safe_select("products")
        products = [p for p in all_products if p.get("quantity",0)>0 and p.get("visible",True) is not False]
        categories = get_categories()
        cat_map = {c["id"]: c["name"] for c in categories}
        for p in products:
            p["category_name"] = cat_map.get(p.get("category_id"), "")
    selected_product = request.args.get("product", "")
    return render_template("store.html", s=s, products=products, categories=categories,
...
# ══════════════════════════════════════
# الأقسام (Categories) — إضافة / تعديل / حذف
# ══════════════════════════════════════
@app.route("/admin/categories")
@admin_required
def admin_categories():
    categories = get_categories()
    products_all = safe_select("products")
    # عدد المنتجات بكل قسم
    for c in categories:
        c["product_count"] = len([p for p in products_all if p.get("category_id")==c["id"]])
    s = get_settings()
    return render_template("admin_categories.html", categories=categories, s=s,
        user=session.get("logged_user"), role=session.get("role"))
...
def add_category():
    ...
    if name:
        ...
        max_order = max([c.get("display_order",0) for c in existing], default=0)
        supabase.table("categories").insert({"name": name, "display_order": max_order+1}).execute()
    return redirect(url_for("admin_categories"))

@app.route("/admin/categories/update/<string:cid>", methods=["POST"])
@admin_required
def update_category(cid):
    name = request.form.get("name","").strip()
    order = request.form.get("display_order", 0)
    if name:
        supabase.table("categories").update({"name": name, "display_order": int(order)}).eq("id", cid).execute()
    return redirect(url_for("admin_categories"))

@app.route("/admin/categories/delete/<string:cid>", methods=["POST"])
@admin_required
def delete_category(cid):
    # المنتجات المرتبطة بهذا القسم تصير "بدون قسم" بدل ما تُحذف
    supabase.table("products").update({"category_id": None}).eq("category_id", cid).execute()
...
def admin_products():
    ...
    cat_map = {c["id"]: c["name"] for c in categories}
    for p in products:
        p["category_name"] = cat_map.get(p.get("category_id"), "بدون قسم")
    s = get_settings()
    customer_url = s.get("customer_url","")
    for p in products:
        if customer_url:
            p["qr"] = generate_qr(f"{customer_url}?product={urllib.parse.quote(p['name'])}")
    return render_template("admin_products.html", products=products, categories=categories, s=s,
        user=session.get("logged_user"), role=session.get("role"))
...
def add_product():
    ...
    if existing:
        ...
        supabase.table("products").insert({"name": p_name, "quantity": qty,
            ...
            "max_order_qty": int(f.get("max_order_qty",qty))}).execute()
...
@app.route("/admin/products/update/<string:pid>", methods=["POST"])
@admin_required
...
@app.route("/admin/products/delete/<string:pid>", methods=["POST"])
@admin_required
def delete_product(pid):
    supabase.table("products").delete().eq("id", pid).execute()
    return redirect(url_for("admin_products"))

@app.route("/admin/products/visibility/<string:pid>", methods=["POST"])
@admin_required
...
@admin_required
def admin_finance():
    wallets   = safe_select("wallets")
    capitals  = safe_select("capital_accounts")
    logs      = safe_select("financial_logs")
    users     = load_users()
    s = get_settings()
...
def admin_settings():
    ...
    users = load_users()
    coupons = []
    try: coupons = safe_select("coupons")
    except: pass
    products_all = safe_select("products")
...
def admin_reports():
    ...
    products     = safe_select("products")
    reviews      = []
    try: reviews = safe_select("reviews")
    except: pass
    s = get_settings()
